...
function withCORS(headers, request) {
    ...
    let headersEmpty = true
    for(var key in headers) {
        if(headers.hasOwnProperty(key))
            headersEmpty = false;
    }
    if(headersEmpty === false){
        if(headers['set-cookie'] !== undefined){
            let sameSiteString = headers['set-cookie'][0]
            console.log(headers['set-cookie'])
            sameSiteString = sameSiteString.replace('Lax', 'None')
            const regexp = /domain=(.*);/g
            // sameSiteString = sameSiteString.replace(regexp, "domain=localhost;")
            console.log(sameSiteString)
            headers['set-cookie']= [sameSiteString]
            console.log(headers['set-cookie'])
        }
    }
    console.log(headers)
    if(request.headers.origin !== undefined){
        headers['access-control-allow-origin'] = 'http://test.mydomain.dot.com:3000';
    }
    else{
        headers['access-control-allow-origin'] = 'http://test.mydomain.dot.com:3000';
    }
...
function proxyRequest(req, res, proxy) {
    ...
    console.log("po " + JSON.stringify(proxyOptions))
    if(req.headers !== undefined)
        console.log("req " + req.headers.host)
    if(res.headers !== undefined){
        console.log("res " + res.headers.host)
        // console.log(res.headers['set-cookie'])
        // let sameSiteString = res.headers['set-cookie'][0]
        // sameSiteString.replace('Lax', 'None')
        // res.headers['set-cookie']= [sameSiteString]
        // console.log(sameSiteString)
        // console.log(res.headers['set-cookie'])
    }
...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        if (options.spoofOrigin) {
            ...
            console.log('spoofing origin as' + targetUrl.protocol + "//" + targetUrl.hostname)
...
const createServer = function createServer(options) {
    ...
    const httpProxyOptions = {
        ...
        cookieDomainRewrite: 'bigbanana123.herokuapp.com'
    };

    if (options.httpProxyOptions) {
        Object.assign(httpProxyOptions, options.httpProxyOptions);
    }
    console.log(httpProxyOptions)
    ...
    proxy.on('error', function proxyErrorHandler(err, req, res) {
        ...
        res.writeHead(404, {
            ...
            'Access-Control-Allow-Origin': 'http://test.mydomain.dot.com:3000'
        });
